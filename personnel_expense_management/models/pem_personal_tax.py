...
class PemPersonalTax(models.Model):
    _name = 'pem.personal.tax'
    _description = "个人所得税"
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = 'create_date desc'

    name = fields.Char('税表编号', required=True, readonly=True, index=True, copy=False, default='新建')
    ...
    total_company_commitment_amount=fields.Float('公司应付税合计',compute='compute_total_commitment_amount', store=True,track_visibility='always', digits=(10, 3))
    total_personal_commitment_amount=fields.Float('个人应付税合计',compute='compute_total_commitment_amount', store=True,track_visibility='always', digits=(10, 3))
    company_id = fields.Many2one('res.company', '公司', readonly=True, default=lambda self: self.env.user.company_id)
    tax_line_ids = fields.One2many('pem.personal.tax.line', 'pem_personal_tax_id', '个人所得税明细', redonly=True,
                                   copy=True)
    ...
    def create(self, vals):
        if vals.get('name', '新建') == '新建':
            vals['name'] = self.env['ir.sequence'].next_by_code('pem.personal.tax') or '/'
        result = super(PemPersonalTax, self).create(vals)
        return result
...
class PemPersonalTaxLine(models.Model):
    _name = 'pem.personal.tax.line'
    _description = "个人所得税明细表"
    ...
    pem_personal_tax_id = fields.Many2one('pem.personal.tax', '个人所得税表单')
